...
int main (int argc, char *argv[])
{
  ...
  boost::interprocess::managed_shared_memory segment(boost::interprocess::open_or_create, "MizhangSharedMemory", 65535);
  ...
  for(int i = 0; i < 500; ++i) {
    ShmHashMap::iterator iter = hash_map1->find(ShmString(to_string(i).c_str(),segment.get_allocator<ShmString>()));
    if (iter != hash_map1->end()) {
      ...
      hash_map1->insert(ValueType(ShmString(to_string(i).c_str(),segment.get_allocator<ShmString>()), ShmString("init empty",segment.get_allocator<ShmString>())));
    }
  }
